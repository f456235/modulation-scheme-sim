...
h1=fir1(30, fp/Fs, boxcar(31));% 生成fir滤波器系统函数中分子多项式的系数
% fir1函数三个参数分别是阶数，数字截止频率，滤波器类型
% 这里是生成了30阶(31个抽头系数)的矩形窗滤波器
[h,w]=freqz(h1, 1,512);      % 生成fir滤波器的频率响应
% freqz函数的三个参数分别是滤波器系统函数的分子多项式的系数，分母多项式的系数(fir滤波器分母系数为1)和采样点数(默认)512
lvbo1=2*fftfilt(h1,jt1);     % 对信号进行滤波，jt1是等待滤波的信号，h是fir滤波器的系统函数的分子多项式系数
lvbo2=2*fftfilt(h1,jt2);     % 对信号进行滤波，jt2是等待滤波的信号，h是fir滤波器的系统函数的分子多项式系数
...
x8=code(fz,:);              % 将原来code的第一行复制L次，称为L*M的矩阵
...
figure(8);                  % 绘制第8幅图
scatter(I,Q);               % 绘制星座图，如果没有16个点，是因为码元数量不够的原因
title('QAM星座图');        % 标题
% 让坐标轴放在坐标原点
ax = gca;
